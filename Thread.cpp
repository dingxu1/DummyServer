...
{ 
	...
	void runInThread()
	{
		...
		Dummy::CurrentThread::t_threadName = name_.empty() ? "DummyThread" : name_.c_str();
		try
		{
			func_();
			Dummy::CurrentThread::t_threadName = "finished";
		}
		catch (...)
		{
			Dummy::CurrentThread::t_threadName = "crashed";
